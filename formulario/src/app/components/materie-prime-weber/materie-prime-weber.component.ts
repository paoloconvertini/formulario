...
import {StoricoDialogWeberComponent} from "../storico-dialog-weber/storico-dialog-weber.component";

@Component({
  selector: 'app-materie-prime',
  templateUrl: './materie-prime-weber.component.html',
  styleUrls: ['./materie-prime-weber.component.css']
})
export class MateriePrimeWeberComponent extends CommonListComponent implements OnInit {

  displayedColumns: string[] = ['nome', 'prezzo', 'u.m.', 'tipologia', 'azioni'];
  isAdmin: boolean = false;
  filtro: Filtro = new Filtro();

  constructor(private service: MateriePrimeWeberService, private dialog: MatDialog) {
    super();
  }
  ...
  apriStorico(materiaPrima: any) {
    const dialogRef = this.dialog.open(StoricoDialogWeberComponent, {
      width: '30%',
      data: materiaPrima,
    });
